...
```verilog
assign ready = 下一阶段的ready | !valid;
always@(posedge clk)
    valid <= ready ? 上一阶段的valid : valid;
```

一些特殊的情况：

1. 当前指令流中存在分支或跳转指令时，IF不接受新的指令
2. 当前指令流中存在写后读冲突时，ID不接受新的指令
3. 只要EX为ready，ID就将valid置0
4. 当DATA_CACHE正在读写数据时，ME不接受新的指令，valid保持
5. 写回阶段总是ready的
6. 写回阶段没有输出到下一阶段的valid信号

上述第1点的目的是解决控制冒险，第2、3点的目的是解决数据冒险。
